...
const languageOptions = [
  ...
  { code: 'zh', label: 'Chinese', nativeLabel: 'Simplified Chinese', region: 'China' },
  { code: 'ja', label: 'Japanese', nativeLabel: 'Japanese', region: 'Japan' },
  { code: 'ko', label: 'Korean', nativeLabel: 'Korean', region: 'Korea' },
  { code: 'es', label: 'Spanish', nativeLabel: 'Spanish', region: 'Spain' },
  { code: 'fr', label: 'French', nativeLabel: 'French', region: 'France' },
  { code: 'de', label: 'German', nativeLabel: 'German', region: 'Germany' },
  { code: 'pt', label: 'Portuguese', nativeLabel: 'Portuguese', region: 'Portugal' },
]
...
const copyByLanguage = {
  ...
  zh: {
    ...
    createAccount: 'Create Account',
    resetPassword: 'Reset Password',
    loginSubtitle: 'Log in to Spotify and continue your music journey.',
    signupSubtitle: 'Create your Spotify account and get started.',
    resetSubtitle: 'Enter your email and new password to reset your account.',
    defineMusic: 'Define Real Music',
    tryIt: 'Start Now',
    users: 'users.',
    aiGraphics: 'AI generated graphics.',
    joinNow: 'Join Now',
  ...
  ja: {
    ...
    createAccount: 'Create Account',
    resetPassword: 'Reset Password',
    loginSubtitle: 'Log in to Spotify and keep listening.',
    signupSubtitle: 'Sign up and start your Spotify experience.',
    resetSubtitle: 'Enter your email and a new password.',
    defineMusic: 'Define True Music',
    tryIt: 'Try It Now',
    users: 'users.',
    aiGraphics: 'AI generated graphics.',
    joinNow: 'Join Now',
  ...
  ko: {
    ...
    createAccount: 'Create Account',
    resetPassword: 'Reset Password',
    loginSubtitle: 'Log in to Spotify and continue listening.',
    signupSubtitle: 'Join Spotify and start exploring.',
    resetSubtitle: 'Enter your email and a new password to continue.',
    defineMusic: 'Define True Music',
    tryIt: 'Try It Now',
    users: 'users.',
    aiGraphics: 'AI generated graphics.',
  ...
  es: {
    ...
    loginSubtitle: 'Inicia sesion en Spotify para continuar.',
    signupSubtitle: 'Registrate para empezar con Spotify.',
    resetSubtitle: 'Introduce tu correo y tu nueva contrasena.',
    defineMusic: 'Define Real Music',
    tryIt: 'Try It Now',
    users: 'users.',
    aiGraphics: 'AI generated graphics.',
  ...
  fr: {
    ...
    loginSubtitle: 'Connectez-vous a Spotify pour continuer.',
    signupSubtitle: 'Inscrivez-vous pour commencer avec Spotify.',
    resetSubtitle: 'Entrez votre e-mail et votre nouveau mot de passe.',
    defineMusic: 'Define Real Music',
    tryIt: 'Try It Now',
    users: 'users.',
    aiGraphics: 'AI generated graphics.',
  ...
  de: {
    ...
    loginSubtitle: 'Melde dich bei Spotify an, um fortzufahren.',
    signupSubtitle: 'Registriere dich, um mit Spotify zu starten.',
    resetSubtitle: 'Gib deine E-Mail und dein neues Passwort ein.',
    defineMusic: 'Define Real Music',
    tryIt: 'Try It Now',
    users: 'users.',
    aiGraphics: 'AI generated graphics.',
  ...
  pt: {
    ...
    loginSubtitle: 'Entre no Spotify para continuar.',
    signupSubtitle: 'Cadastre-se para comecar no Spotify.',
    resetSubtitle: 'Informe seu e-mail e sua nova senha.',
    defineMusic: 'Define Real Music',
    tryIt: 'Try It Now',
    users: 'users.',
    aiGraphics: 'AI generated graphics.',
...
const kickedOutBanner = ref(false)
const signUpSuccessMessage = ref('')
let signUpRedirectTimer = null
...
function clearSignUpSuccess() {
  ...
  if (signUpRedirectTimer) {
    clearTimeout(signUpRedirectTimer)
    signUpRedirectTimer = null
  }
}
...
async function handleLogin() {
  ...
  try {
    ...
    if (data) {
      ...
      if (data.userInfo?.jwt) {
        sessionStorage.setItem('jwt', data.userInfo.jwt)
        localStorage.setItem('jwt', data.userInfo.jwt)
      }
      if (data.userInfo?.refreshToken) {
        sessionStorage.setItem('refreshToken', data.userInfo.refreshToken)
        localStorage.setItem('refreshToken', data.userInfo.refreshToken)
      }
    }
    window.location.href = '/spotify-frontend/main.html'
  } catch (error) {
...
async function handleSignUpSubmit() {
  ...
  try {
    ...
    signUpSuccessMessage.value = message || 'Sign up successful! Redirecting to login...'
    email.value = ''
    username.value = ''
    nickname.value = ''
    password.value = ''
...
function isJwtExpired(token) {
  try {
    ...
    // exp 是秒级时间戳，比当前秒数小就是过期
    return payload.exp * 1000 < Date.now()
  } catch {
    return true // 解析失败视为过期，让用户重新登录
  }
}
...
onMounted(() => {
  ...
  // 检测是否被其他设备登录踢出
  if (sessionStorage.getItem('kickedOut') === '1') {
    sessionStorage.removeItem('kickedOut')
    kickedOutBanner.value = true
    setTimeout(() => { kickedOutBanner.value = false }, 4500)
  }
  // 检查是否已登录，如果有有效的 JWT 则直接跳转到主页面
  const jwt = localStorage.getItem('jwt') || sessionStorage.getItem('jwt')
  if (jwt && !isJwtExpired(jwt)) {
    // 同步到 sessionStorage 供其他页面使用
...
<template>
  <div class="auth-page" v-cloak>
    <div v-if="kickedOutBanner" class="kicked-out-banner">Logged out</div>
    <div class="left-panel">
      ...
      <div v-if="!isForgotPasswordMode">
        <div class="form-group">
          <div v-if="signUpSuccessMessage" class="form-success-text">{{ signUpSuccessMessage }}</div>
